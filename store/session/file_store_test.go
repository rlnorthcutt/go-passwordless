...
func TestFileStore(t *testing.T) {
	...
	filePath := "./test_session_data"
	fs := session.NewFileStore(filePath, secretKey)

	// Clean up test directory after the tests
	...
	t.Run("PrimaryFlow", func(t *testing.T) {
		...
		t.Run("VerifyDeletionAfterUse", func(t *testing.T) {
			...
			token, err := fs.Exists(ctx, tokenID)
